...
#data_path = '/Users/mulugetasemework/Dropbox/R/OtherCSVs/1MetaTable1_filtered3_no_ttest.csv'
...
pick_special_predictive_columns = 0
remove_eye_pos_neurons = 0
...
if remove_eye_pos_neurons == 1:
    #drop eye pos and other irrelevant rows
    TDData = TDData[(TDData.baseline_pre_vs_eyepos_max>0.05) & \
                    (TDData.baseline_pre_vs_eyepos_median>0.05) & \
                    (TDData.baseline_pre_vs_eyepos_mean>0.05)]

drop_these_columns = ['TaskType','visual_vs_baseline_ttest_h',
                  'new_saccade_baseline_max_ranksum']
...
#to be saved for other codes (after imp is done)
target_continuous = pd.DataFrame(to_be_predicted)
...
if pick_special_predictive_columns == 1:
    print("using special predictive features only")
...
#%% data
X = TDData.loc[:, TDData.columns != 'memory_response_strength']#.iloc[:,:TDData.shape[1]-1]
y = to_be_predicted
#print(np.unique(X))
...
if do_over_all_var_importance == 1:
   ...
   imp_data = pd.DataFrame(imp_cols_data)
